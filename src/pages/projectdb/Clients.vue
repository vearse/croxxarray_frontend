    ...
    >
      ...
      <validation-observer ref="form" v-slot="{ invalid, handleSubmit }">
          <form action="" @submit.prevent="form.editmode ? handleSubmit(updateComponentRecord) : handleSubmit(createNewClient)"> 
              ...
              <CRow>
                <CCol sm="12">
                   <validation-provider  name="QCP IP"  rules="max:512"
                    v-slot="validationContext">
                      <CTextarea
                        v-model="form.qcp_ip"
                        :invalid-feedback="validationContext.errors[0]"
...
export default {
  ...
  methods: {
    ...
    createNewClient(){
      this.$store.dispatch('clients/create', this.form)
      .then(result => {
    ...
    updateComponentRecord(){
       let vm = this;
        this.$store.dispatch('clients/update', { id:this.form.id, payload: this.form}) 
        .then(result => {
          if (this.success !== false && this.error === false) {
            this.loadRecords();
            this.createModal = false;
