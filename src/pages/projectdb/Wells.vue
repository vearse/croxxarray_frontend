                        ...
                        <table class="table table-hover">
                          <thead>
                            <tr>
                              ...
                              <th class="inline-table-head"><div>Well Type</div></th>
                              <th class="inline-table-head"><div>Platform</div></th>
                          ...
                          <tbody class="position-relative">
                            ...
                            <tr v-else
                                ...
                                <td @click="editItem(item)" class=""> {{item.well_type}} </td> 
                                <td @click="editItem(item)" class=""> <span v-if="item.platform"> {{item.platform.name}}</span> </td>
    ...
    >
      ...
      <validation-observer ref="form" v-slot="{ invalid, handleSubmit }">
          <form action="" @submit.prevent="form.editmode ? handleSubmit(updateComponentRecord) : handleSubmit(createNewComponent)"> 
              ...
              <CRow>
                <CCol sm="12">
                   <validation-provider  name="Well Type" rules="required"
                    v-slot="validationContext">
                      <CInput
                        v-model="form.well_type" label="Well Type"  
                        :invalid-feedback="validationContext.errors[0]"
                        :class="{ 'is-invalid': validationContext.errors[0] }"
                        placeholder="Enter Well Type"
                      />
                    <span class="text-error" v-if="validationContext.errors[0]">{{validationContext.errors[0]}}</span>
                  </validation-provider>
                </CCol> 
              ...
              <CRow>
                <CCol sm="12">
                    <validation-provider name="Role Description" rules="required"
                      v-slot="validationContext">
                        <label for="uid-mu3370ovqhi" class=""> Platform </label>
                        <select 
                          :invalid="validationContext.errors[0]"
                          :class="{ 'is-invalid': validationContext.errors[0] }"
                          v-model="form.platform_id" id="uid-mu3370ovqhi" vertical="" class="form-control">
                            <option value="" selected="selected" hidden="hidden"> Please select </option> 
                            <option :key="platform.id" :value="platform.id" v-for="platform in platforms" >{{platform.name}} </option>
                        </select>
                    </validation-provider>
                </CCol> 
              </CRow>
...
export default {
  ...
  data () {
    return {
      ...
      platforms: Object.values(this.$store.state.platforms.dataList),
      form: {
        name: '',
        description: '',
        well_type: '',
  ...
  methods: {
    ...
    loadRecords(){
      ...
      this.$store.dispatch('wells/list', payload);
    },
    ...
    loadPlatformsRecord(){
      ...
      this.$store.dispatch('platforms/list', payload);
    },
    ...
    createNewComponent(){
      this.$store.dispatch('wells/create', this.form)
      .then(() => {
          if (this.success !== false && this.error === false) {
            ...
            this.form.setErrors(this.validationErrors); // set VeeValidation error
    ...
    updateComponentRecord(){
       let vm = this;
        this.$store.dispatch('wells/update', { id:this.form.id, payload: this.form}) 
        .then(result => {
          if (this.success !== false && this.error === false) {
            this.loadRecords();
            this.createModal = false;
