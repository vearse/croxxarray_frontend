                        ...
                        <table class="table table-hover">
                          <thead>
                            <tr>
                              ...
                              <th class="inline-table-head"><div>field</div></th>
                          ...
                          <tbody class="position-relative">
                            ...
                            <tr v-else
                                ...
                                <td @click="editItem(item)" class=""> <span v-if="item.field"> {{item.field.name}}</span> </td>
              ...
              <CRow>
                <CCol sm="12">
                    <validation-provider name="Role Description" rules="required"
                      v-slot="validationContext">
                        <label for="uid-mu3370ovqhi" class=""> Field </label>
                        <select 
                          :invalid="validationContext.errors[0]"
                          :class="{ 'is-invalid': validationContext.errors[0] }"
                          v-model="form.field_id" id="uid-mu3370ovqhi" vertical="" class="form-control">
                            <option value="" selected="selected" hidden="hidden"> Please select </option> 
                            <option :key="field.id" :value="field.id" v-for="field in fields" >{{field.name}} </option>
                        </select>
                    </validation-provider>
                </CCol> 
              </CRow>
...
export default {
  ...
  data () {
    return {
      ...
      fields: Object.values(this.$store.state.fields.dataList),
      form: {
        name: '',
        description: '',
        field_id: '',
  ...
  methods: {
    ...
    createNewComponent(){
      this.$store.dispatch('platforms/create', this.form)
      .then(() => {
    ...
    updateComponentRecord(){
       let vm = this;
        this.$store.dispatch('platforms/update', { id:this.form.id, payload: this.form}) 
        .then(result => {
          if (this.success !== false && this.error === false) {
            this.loadRecords();
            this.createModal = false;
