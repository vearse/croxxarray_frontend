...
export default {
  ...
  methods: {
    loadRecords(){
      ...
      this.$store.dispatch('completionTypes/list', payload);
    },
    ...
    createNewClient(){
      this.$store.dispatch('completionTypes/create', this.form)
      .then(result => {
    ...
    updateComponentRecord(){
       let vm = this;
        this.$store.dispatch('completionTypes/update', { id:this.form.id, payload: this.form}) 
        .then(result => {
          if (this.success !== false && this.error === false) {
            this.loadRecords();
            this.createModal = false;
